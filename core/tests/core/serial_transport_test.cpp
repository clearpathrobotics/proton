...
// -----------------------------------------------------------------------
// proton_fill_frame_header
// -----------------------------------------------------------------------

TEST(FillFrameHeader, NullHeaderReturnsError)
{
  EXPECT_EQ(proton_fill_frame_header(nullptr, 10), PROTON_NULL_PTR_ERROR);
}

TEST(FillFrameHeader, MagicBytesAreCorrect)
{
  uint8_t header[4] = {};
  ASSERT_EQ(proton_fill_frame_header(header, 0), PROTON_OK);
...
{
  uint8_t header[4] = {};
  ASSERT_EQ(proton_fill_frame_header(header, 0x0203), PROTON_OK);
...
{
  uint8_t header[4] = {};
  ASSERT_EQ(proton_fill_frame_header(header, 0xFFFF), PROTON_OK);
...
{
  uint8_t crc[2] = {};
  EXPECT_EQ(proton_fill_crc16(nullptr, 0, crc), PROTON_NULL_PTR_ERROR);
}

TEST(FillCrc16, NullCrcOutputReturnsError)
{
  const uint8_t payload[] = {0x01};
  EXPECT_EQ(proton_fill_crc16(payload, 1, nullptr), PROTON_NULL_PTR_ERROR);
}
...
TEST(FillCrc16, EmptyPayloadCrcIsAllOnes)
{
  ...
  uint8_t crc[2] = {};
  ASSERT_EQ(proton_fill_crc16(dummy, 0, crc), PROTON_OK);
...
TEST(FillCrc16, SingleByteKnownValue)
{
  ...
  uint8_t crc[2] = {};
  ASSERT_EQ(proton_fill_crc16(payload, 1, crc), PROTON_OK);
...
TEST(FillCrc16, DeterministicOutput)
{
  ...
  uint8_t crc_a[2] = {};
  uint8_t crc_b[2] = {};
  ASSERT_EQ(proton_fill_crc16(payload, sizeof(payload), crc_a), PROTON_OK);
  ASSERT_EQ(proton_fill_crc16(payload, sizeof(payload), crc_b), PROTON_OK);
...
TEST(FillCrc16, ByteOrderAffectsCrc)
{
  ...
  uint8_t crc_a[2] = {};
  uint8_t crc_b[2] = {};
  ASSERT_EQ(proton_fill_crc16(payload_a, sizeof(payload_a), crc_a), PROTON_OK);
  ASSERT_EQ(proton_fill_crc16(payload_b, sizeof(payload_b), crc_b), PROTON_OK);
  EXPECT_FALSE(crc_a[0] == crc_b[0] && crc_a[1] == crc_b[1]);
}

// -----------------------------------------------------------------------
// proton_check_framed_payload
// -----------------------------------------------------------------------

TEST(CheckFramedPayload, NullPayloadReturnsError)
{
  EXPECT_EQ(proton_check_framed_payload(nullptr, 0, 0xFFFF), PROTON_NULL_PTR_ERROR);
}
...
TEST(CheckFramedPayload, CorrectCrcReturnsOk)
{
  ...
  const uint8_t payload[] = {0xAB};
  EXPECT_EQ(proton_check_framed_payload(payload, 1, 0xE571), PROTON_OK);
}

TEST(CheckFramedPayload, WrongCrcReturnsCrcError)
{
  const uint8_t payload[] = {0xAB};
  EXPECT_EQ(proton_check_framed_payload(payload, 1, 0x0000), PROTON_CRC16_ERROR);
}

TEST(CheckFramedPayload, EmptyPayloadCorrectCrc)
{
  const uint8_t dummy[1] = {};
  EXPECT_EQ(proton_check_framed_payload(dummy, 0, 0xFFFF), PROTON_OK);
}

TEST(CheckFramedPayload, EmptyPayloadWrongCrc)
{
  const uint8_t dummy[1] = {};
  EXPECT_EQ(proton_check_framed_payload(dummy, 0, 0x0000), PROTON_CRC16_ERROR);
}

// -----------------------------------------------------------------------
// proton_get_framed_payload_length
// -----------------------------------------------------------------------

TEST(GetFramedPayloadLength, NullBufReturnsError)
{
  uint16_t length = 0;
  EXPECT_EQ(proton_get_framed_payload_length(nullptr, &length), PROTON_NULL_PTR_ERROR);
}
...
TEST(GetFramedPayloadLength, NullLengthOutputReturnsError)
{
  ...
  EXPECT_EQ(proton_get_framed_payload_length(frame, nullptr), PROTON_NULL_PTR_ERROR);
}
...
TEST(GetFramedPayloadLength, WrongFirstMagicByteReturnsError)
{
  ...
  uint16_t length = 0;
  EXPECT_EQ(proton_get_framed_payload_length(frame, &length), PROTON_INVALID_HEADER_ERROR);
}
...
TEST(GetFramedPayloadLength, ValidHeaderReturnsCorrectLength)
{
  ...
  uint16_t length = 0;
  ASSERT_EQ(proton_get_framed_payload_length(frame, &length), PROTON_OK);
  EXPECT_EQ(length, 42);
}
...
TEST(SerialFraming, FillHeaderThenGetLength_RoundTrip)
{
  ...
  ASSERT_EQ(proton_fill_frame_header(header, expected_len), PROTON_OK);

  uint16_t decoded_len = 0;
  ASSERT_EQ(proton_get_framed_payload_length(header, &decoded_len), PROTON_OK);
...
TEST(SerialFraming, FillCrcThenCheckCrc_RoundTrip)
{
  ...
  uint8_t crc_bytes[2] = {};
  ASSERT_EQ(proton_fill_crc16(payload, sizeof(payload), crc_bytes), PROTON_OK);

  const uint16_t crc_val = (uint16_t)crc_bytes[0] | ((uint16_t)crc_bytes[1] << 8);
  EXPECT_EQ(proton_check_framed_payload(payload, sizeof(payload), crc_val), PROTON_OK);
}
...
TEST(SerialFraming, ModifiedPayloadFailsCrcCheck)
{
  ...
  EXPECT_EQ(proton_check_framed_payload(payload, sizeof(payload), crc_val), PROTON_CRC16_ERROR);
}
