...
Node::Node(const std::string config_file, const std::string name, bool auto_configure,
           bool auto_activate)
...
proton_status_e Node::configure() {
  ...
  for (auto& connection_config : config_.getConnections()) {
    ...
    connections_.emplace(
        peer, Connection(node_config_, config_.getNodes().at(peer), connection_config,
                         std::bind(&Node::readCompleteCallback, this, std::placeholders::_1,
                                   std::placeholders::_2)));
  }
...
proton_status_e
Node::registerCallback(const std::string& bundle_name, BundleHandle::BundleCallback callback) {
...
proton_status_e Node::registerHeartbeatCallback(const std::string& producer,
...
Connection::Connection(const NodeConfig& node_config, const NodeConfig& peer_config,
                       const ConnectionConfig& connection_config, ReadCompleteCallback callback)
    : config_(peer_config)
    , callback_(callback)
    , state_(PROTON_NODE_UNCONFIGURED)
    , rx_kbps_(0.0)
    , tx_kbps_(0.0) {
  ...
  EndpointConfig node_endpoint_config =
      node_config.endpoints.at(node_connection_endpoint_config.id);
  EndpointConfig peer_endpoint_config =
      peer_config.endpoints.at(peer_connection_endpoint_config.id);
  ...
  if (transport_type_ == transport_types::UDP4) {
    auto node_endpoint =
        proton::socket_endpoint(node_endpoint_config.ip, node_endpoint_config.port);
    auto peer_endpoint =
        proton::socket_endpoint(peer_endpoint_config.ip, peer_endpoint_config.port);
    setTransport(std::make_unique<Udp4Transport>(node_endpoint, peer_endpoint));
  ...
  while (1) {
    ...
    switch (getTransportState()) {
      ...
      case PROTON_TRANSPORT_CONNECTED: {
        ...
        if (status != PROTON_OK) {
          std::cout << "Failed to poll for bundle on peer " << config_.name << ": " << status
                    << std::endl;
        }
        break;
      }
      ...
      case PROTON_TRANSPORT_ERROR: {
        status = disconnect();
        if (status != PROTON_OK) {
          std::cout << "Failed to disconnect from peer " << config_.name << ": " << status
                    << std::endl;
        }
        std::this_thread::sleep_for(std::chrono::seconds(1));
      }
