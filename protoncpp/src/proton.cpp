...
Node::Node(
  const std::string config_file, const std::string name, bool auto_configure, bool auto_activate)
...
proton_status_e Node::configure()
{
  ...
  for (auto & connection_config : config_.getConnections())
  {
    ...
    connections_.emplace(
      peer, Connection(
              node_config_, config_.getNodes().at(peer), connection_config,
              std::bind(
...
void Node::startStatsThread() { stats_thread_ = std::thread(&Node::runStatsThread, this); }
  ...
  tx_kbps_(0.0)
{
  ...
  if (
    connection_config.connection.first.node == node_config.name &&
    connection_config.connection.second.node == peer_config.name)
  {
    ...
    connection_config.connection.second.node == node_config.name &&
