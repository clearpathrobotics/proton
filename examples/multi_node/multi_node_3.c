...
double rx1, tx1, rx2, tx2;

uint32_t last_node1_heartbeat, last_node2_heartbeat = 0;
...
void proton_bundle_time_callback() {
  cb_counts[CALLBACK_TIME]++;
}

void proton_bundle_node2_heartbeat_callback()
{
  cb_counts[CALLBACK_HEARTBEAT_NODE2]++;
  node3_node.peers[PROTON__PEER__NODE2].state = PROTON_NODE_ACTIVE;
...
void proton_bundle_node1_heartbeat_callback()
{
  ...
  node3_node.peers[PROTON__PEER__NODE1].state = PROTON_NODE_ACTIVE;
  last_node1_heartbeat = time(NULL);
}

bool proton_node_node2_transport_connect() {
  sock_recv = socket_init(PROTON__NODE__NODE3__ENDPOINT__1__IPHL, PROTON__NODE__NODE3__ENDPOINT__1__PORT, true);
  sock_send = socket_init(PROTON__NODE__NODE2__ENDPOINT__1__IPHL, PROTON__NODE__NODE2__ENDPOINT__1__PORT, false);
...
bool proton_node_node1_transport_connect() {
  serial_port = serial_init(PROTON__NODE__NODE1__ENDPOINT__1__DEVICE);
...
size_t proton_node_node1_transport_read(uint8_t *buf, size_t len) {
  ...
  if (bytes_read > 0)
  {
    rx1 += bytes_read + PROTON_FRAME_OVERHEAD;
  }
...
size_t proton_node_node1_transport_write(const uint8_t *buf, size_t len) {
  ...
  if (bytes_written > 0)
  {
    tx1 += bytes_written + PROTON_FRAME_OVERHEAD;
  }
...
void *timer_1hz(void *arg) {
  ...
  while (1) {
    ...
    {
      node3_node.peers[PROTON__PEER__NODE2].state = PROTON_NODE_INACTIVE;
    }

    if (time(NULL) - last_node1_heartbeat > PROTON__NODE__NODE1__HEARTBEAT__PERIOD / 1000)
    {
      node3_node.peers[PROTON__PEER__NODE1].state = PROTON_NODE_INACTIVE;
    }
...
void * stats(void *arg) {
  ...
  while (1) {
    ...
    printf("  State: %u, Transport: %u\r\n", node3_node.peers[PROTON__PEER__NODE1].state, node3_node.peers[PROTON__PEER__NODE1].transport.state);
    printf("  Rx: %.3lf KB/s Tx: %.3lf KB/s\r\n", rx1 / 1000, tx1 / 1000);
    printf("Peer: %s (%s)\r\n", PROTON__NODE__NODE2__NAME, PROTON__NODE__NODE2__ENDPOINT__1__TYPE);
    printf("  State: %u, Transport: %u\r\n", node3_node.peers[PROTON__PEER__NODE2].state, node3_node.peers[PROTON__PEER__NODE2].transport.state);
    printf("  Rx: %.3lf KB/s Tx: %.3lf KB/s\r\n", rx2 / 1000, tx2 / 1000);
...
void * spin_node2()
{
  proton_spin(&node3_node, PROTON__PEER__NODE2);
}
